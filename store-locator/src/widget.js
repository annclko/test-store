...
function getApiUrl() {
  return window.__STORE_LOCATOR_API_URL__ || DEFAULT_API_URL;
}
...
// Load CSS (now handled by webpack, but keeping function for compatibility)
function loadCSS() {
  // CSS is now bundled by webpack, so this is a no-op
...
async function loadStores() {
  ...
  try {
    ...
    console.log('Fetching stores from:', `${apiUrl}/locations`);
    ...
    const apiUrl = getApiUrl();
    listEl.innerHTML =
      `<div class="error">Error loading stores: ${error.message}<br>Make sure the server is running on ${apiUrl}</div>`;
  }
}
...
// Auto-initialize when DOM is ready (only if not using the module import)
